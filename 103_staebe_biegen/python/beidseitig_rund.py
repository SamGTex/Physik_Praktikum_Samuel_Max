...
d = np.mean(d_stange)
err_d = sem(d_stange)
d_wert = ufloat(d, err_d)

r_wert = (d_wert/2) / 1000 #meter
r = (d/2) / 1000
...
# Achsenbeschriftung
plt.xlabel(r'$(3 L^{2} x - (4 x^3)) \:/\: m^3$')
plt.ylabel(r'$D \:/\: mm$')
...
# Speicherort
plt.savefig('plot_beidseitig_rund_links.pdf')
...
# Speicherort
plt.savefig('plot_beidseitig_rund_rechts.pdf')
