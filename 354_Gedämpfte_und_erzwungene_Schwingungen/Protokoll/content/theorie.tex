...
\end{equation*}
löst die Differentialgleichung \eqref{eqn:dgl_lrc}.
...
Nach einsetzen von $\tilde{\omega}_{1,2}$ \eqref{eqn:lrcomega}, folgt
\begin{equation}
...
\subsubsection{Fallunterscheidung: Lösungsgleichungen}
...
\begin{equation}
    I(t) = A \mathrm{e}^{-\frac{R}{2L}t} = A \mathrm{e}^{-\frac{1}{\sqrt{LC}}t} .
\end{equation}
...
In Abb. \ref{fig:erzw_schwingkreis} ist der Schaltplan für eine erzwungene Schwingung dargestellt.
...
\end{equation*}
lösen. Nach Einsetzen in die DGL \eqref{eqn:dgl_erzwungen} und auflösen nach $U$ ergibt sich
\begin{equation}
...
Die Resonanz kann durch die Breite der Kurve \eqref{eqn:spannung_c} quantifiziert werden.
...
Ist die Phase gerade $\frac{\pi}{4}$ oder $\frac{3}{4}\pi$, so gilt
\begin{equation}
...
\begin{equation}
    ...
    \label{eqn:breite}
\end{equation}
genauso wie bei der schwachen Dämpfung \eqref{eqn:guete1}.