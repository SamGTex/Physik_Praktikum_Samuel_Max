...
T8x_mini = [37, 236, 446, 650, 848, 1051, 1254, 1453]
T7x_mini = T7x_mini[0]
T7x_mini= T7x_mini[1:]
#T8x_mini = T8x_mini[]
...
T8x_max = np.array([168, 364, 567, 756, 955, 1151, 1350, 1550])
T7x_max = T7x_max[0]
T7_max = T7[T7x_max]
T8_max = T8[T8x_max]
...
plt.plot(s, y1_mini, '--', label='Interpolation der Minima von T7')
plt.plot(s, y2_mini, '--', label='Interpolation der Minima von T8')
plt.legend(loc='best')
plt.savefig('dynamisch_T7_T8.pdf')
plt.show


phase = s[T8x_max] - s[T7x_max]
periode = 80
print('phase : ', phase)
A8 = T8_max - y2_mini[T8x_max]
A7 = T7_max - y1_mini[T7x_max]
kappa = (8000*400 * (0.03)**2) /(2 * phase * np.log(A7/A8))
print('kappa : ', kappa)
print('Edelstahl : ', (np.mean(kappa), sem(kappa)))
